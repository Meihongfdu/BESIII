...
cmt_local_setup_GammaConv = $(bin)setup_GammaConv$$$$.make
...
$(cmt_local_tagfile_GammaConv) :
	...
	$(echo) "(constituents.make) Rebuilding $(cmt_final_setup_GammaConv)"; \
	  test ! -f $(cmt_local_setup_GammaConv) || \rm -f $(cmt_local_setup_GammaConv); \
	  trap '\rm -f $(cmt_local_setup_GammaConv)' 0 1 2 15; \
	  $(cmtexe) -tag=$(tags) $(GammaConv_extratags) show setup >$(cmt_local_setup_GammaConv) && \
	  if [ -f $(cmt_final_setup_GammaConv) ] && \
	    \cmp -s $(cmt_final_setup_GammaConv) $(cmt_local_setup_GammaConv); then \
	    \rm $(cmt_local_setup_GammaConv); else \
	    \mv -f $(cmt_local_setup_GammaConv) $(cmt_final_setup_GammaConv); fi
...
not_GammaConv_dependencies = { n=0; for p in $?; do m=0; for d in $(GammaConv_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
GammaConvdirs :
	@if test ! -d $(bin)GammaConv; then $(mkdir) -p $(bin)GammaConv; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)GammaConv
else
GammaConvdirs : ;
endif
...
#	$(echo) "STRUCTURED_OUTPUT="$(bin)GammaConv
#
#$(cmt_local_GammaConv_makefile) :: $(GammaConv_dependencies) $(cmt_local_tagfile_GammaConv) build_library_links dirs GammaConvdirs
#else
...
ifndef QUICK
$(cmt_local_GammaConv_makefile) : $(GammaConv_dependencies) build_library_links
	$(echo) "(constituents.make) Building GammaConv.make"; \
	  $(cmtexe) -tag=$(tags) $(GammaConv_extratags) build constituent_config -out=$(cmt_local_GammaConv_makefile) GammaConv
else
$(cmt_local_GammaConv_makefile) : $(GammaConv_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_GammaConv) ] || \
	  [ ! -f $(cmt_final_setup_GammaConv) ] || \
	  $(not_GammaConv_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building GammaConv.make"; \
	  $(cmtexe) -tag=$(tags) $(GammaConv_extratags) build constituent_config -out=$(cmt_local_GammaConv_makefile) GammaConv; \
	  fi
endif
...
$(cmt_local_GammaConv_makefile) : $(GammaConv_dependencies) build_library_links
	$(echo) "(constituents.make) Building GammaConv.make"; \
	  $(cmtexe) -f=$(bin)GammaConv.in -tag=$(tags) $(GammaConv_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_GammaConv_makefile) GammaConv
else
$(cmt_local_GammaConv_makefile) : $(GammaConv_dependencies) $(cmt_build_library_linksstamp) $(bin)GammaConv.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_GammaConv) ] || \
	  [ ! -f $(cmt_final_setup_GammaConv) ] || \
	  $(not_GammaConv_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
GammaConv :: $(GammaConv_dependencies) $(cmt_local_GammaConv_makefile) dirs GammaConvdirs
	$(echo) "(constituents.make) Starting GammaConv"
	@if test -f $(cmt_local_GammaConv_makefile); then \
	  $(MAKE) -f $(cmt_local_GammaConv_makefile) GammaConv; \
	  fi
#	@$(MAKE) -f $(cmt_local_GammaConv_makefile) GammaConv
...
GammaConvclean :: $(GammaConvclean_dependencies) ##$(cmt_local_GammaConv_makefile)
	...
	@-if test -f $(cmt_local_GammaConv_makefile); then \
	  $(MAKE) -f $(cmt_local_GammaConv_makefile) GammaConvclean; \
	fi
	$(echo) "(constituents.make) GammaConvclean done"
#	@-$(MAKE) -f $(cmt_local_GammaConv_makefile) GammaConvclean
...
GammaConvinstall :: $(GammaConv_dependencies) $(cmt_local_GammaConv_makefile)
	...
	@-$(MAKE) -f $(cmt_local_GammaConv_makefile) install
	$(echo) "(constituents.make) install GammaConv done"

uninstall : GammaConvuninstall

$(foreach d,$(GammaConv_dependencies),$(eval $(d)uninstall_dependencies += GammaConvuninstall))

GammaConvuninstall : $(GammaConvuninstall_dependencies) ##$(cmt_local_GammaConv_makefile)
	$(echo) "(constituents.make) Starting uninstall GammaConv"
	@if test -f $(cmt_local_GammaConv_makefile); then \
	  $(MAKE) -f $(cmt_local_GammaConv_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_GammaConv_makefile) uninstall
	$(echo) "(constituents.make) uninstall GammaConv done"

remove_library_links :: GammaConvuninstall
