...
DssDss::DssDss(const std::string& name, ISvcLocator* pSvcLocator) :
...
StatusCode DssDss::initialize(){
  ...
  NTuplePtr nt1(ntupleSvc(), "FILE1/tag");
	...
	status = m_tuple1->addItem ("mDss",                    mDss_tag_temp1);
...
/////////////////////////////////////////
  NTuplePtr nt2(ntupleSvc(), "FILE1/signal");
	...
	status = m_tuple2->addItem ("mDss_tag",                 mDss_tag_temp2);
...
//if(pdgid==9010221){// f_0
//HepLorentzVector  P4_1=(*iter_mc)->initialFourMomentum();
//P4_1.boost(-0.011,0,0);
//Ta4_11.push_back(P4_1);
//}
}
if(p_mid==-431||p_m2id==-431||p_m3id==-431||p_m4id==-431||p_m5id==-431){
...
double mBC_tag=999;
double maxangle=999;
HepLorentzVector v4_1,v4_2,v4_3,v4_4,v4_5,PSTDs,PSTDss;
Vint tagid;tagid.clear();
Vtrack Tag_trk;Tag_trk.clear();
////////////////////
double delE_ini=9999;
double i_tag=9999;
double k_tag=9999;
double j_tag=9999;
double l_tag=9999;
...
HepLorentzVector pDss = Kvector[i]+Kvector[j]+Pivector[k]+Pi0vector[l];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);

    double pDsstag=pDss.rho();
    double temp1 = (ecm/2)*(ecm/2)-pDsstag*pDsstag ;
    if(temp1<0) temp1 =0;
    double mass_bc  = sqrt(temp1);
    if(mass_bc<2.2||mass_bc>2.4) continue;//Dss mass 2.317
...
mDs_tag=pDs.m();
mDss_tag=pDss.m();
cout<<"mDss_tag=pDss.m();" <<mDss_tag<<endl;
mBC_tag=mass_bc;
PSTDs=pDs;
PSTDss=pDss;
...
idtemp.push_back(Pi0id1[l]);
idtemp.push_back(Pi0id2[l]);
idtemp.push_back(Pi0id1[m]);
idtemp.push_back(Pi0id2[m]);
if(Same(idtemp))continue;

if(Pivector[k].rho()<0.1)continue;
if(Pi0vector[l].rho()<0.1)continue;

HepLorentzVector pDss = Kvector[i]+Kvector[j]+Pivector[k]+Pi0vector[l]+Pi0vector[m];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
    ...
    if(temp1<0) temp1 =0;
    double mass_bc  = sqrt(temp1);
	if(mass_bc<2.20||mass_bc>2.40) continue;
...
tagid.push_back(Pi0id1[l]);
tagid.push_back(Pi0id2[l]);
tagid.push_back(Pi0id1[m]);
tagid.push_back(Pi0id2[m]);
...
HepLorentzVector pDss = Pivector[i]+Pivector[j]+Pivector[k]+Pi0vector[l];
...
HepLorentzVector pDss = Ksovector[i]+Kvector[j]+Pi0vector[k];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
...
HepLorentzVector pDss = Ksovector[i]+Kvector[j]+Pi0vector[k]+Pi0vector[l];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
    double pDsstag=pDss.rho();
...
HepLorentzVector pDss = Kvector[i]+Pivector[j]+Pivector[k]+Pi0vector[l];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
...
HepLorentzVector pDss = Ksovector[i]+Ksovector[j]+Pivector[k]+Pi0vector[l];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
...
HepLorentzVector pDss = Ksovector[i]+Kvector[j]+Pivector[k]+Pivector[l]+Pi0vector[m];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
    ...
    if(temp1<0) temp1 =0;
    double mass_bc  = sqrt(temp1);
    if(mass_bc<2.20||mass_bc>2.4) continue;
...
if(Same(idtemp))continue;
        ...
        HepLorentzVector pDss = Etavector[i]+Pivector[j]+Pi0vector[k];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
        delE_ini = abs(deltaE_tag);
...
HepLorentzVector pDss = Pivector[i]+Pivector[j]+Pivector[k]+Pi0vector[l]+Pi0vector[m];
double deltaE_tag = ecm/2 - pDss.e();
if(fabs(deltaE_tag)<delE_ini){
    delE_ini = abs(deltaE_tag);
...
HepLorentzVector pDss =Pi0vector[m]+Pivector[i]+Pivector[j]+Pivector[k]+Etavector[l];
...
HepLorentzVector pDss = Pi0vector[m]+Pivector[i]+Pivector[j]+Pivector[k]+Gamvector[l];
...
HepLorentzVector pDss =Pi0vector[l]+Etavector[i]+Pivector[j]+Pi0vector[k];
...
if(findtag) {
 ...
 tagcharge_temp1             =     tagcharge;
  ...
  mDss_tag_temp1            =      mDss_tag;
     cout<<"mDss_tag=="<<mDss_tag<<endl;
...
//if(Same(idtemp))continue;
Pgtag=PSTDss;
Pgtag.setE(sqrt(PSTDss.rho()*PSTDss.rho()+2.3178*2.3178));
//v4_6_tem=Gamvector[ga];
//tem1=true;
//}
...
for(int recmode=0;recmode<2;recmode++){
//////
double chicut=999.;
bool findsig=false;
double mm2m=999;
...
//////
if(recmode==0){ //Ds+ -> k+ k- pi+ 

for(int i=0;i<knumber;i++){//for k+
    if(Kcharge[i]!=tagcharge)continue;
for(int k=0;k<knumber;k++){//for k-
    if(Kcharge[k]==tagcharge)continue;
for(int j=0;j<pinumber;j++){//for pi+
if(Picharge[j]!=tagcharge)continue;
for(int l=0;l<pi0number;l++){
...
//HepLorentzVector Pmiss=ECMS-Pgtag_final-Pivector[i]-Pivector[k]-Muvector[j]; 
...
m123=(Kvector[i]+Kvector[k]+Pivector[j]).m();//Ds
m1234=(Kvector[i]+Kvector[k]+Pivector[j]+Pi0vector[l]).m();//Ds
m12=(Kvector[i]+Kvector[k]).m();
m13=(Kvector[i]+Pivector[j]).m();
m23=(Kvector[k]+Pivector[j]).m();
...
TotID=tagid;
//TotID=Dstid;
TotID.push_back(Kid[i]);
TotID.push_back(Kid[k]);
TotID.push_back(Piid[j]);
TotID.push_back(Pi0id1[l]);
TotID.push_back(Pi0id2[l]);
//////////
/*
...
kmfit1->AddTrack(0, Trk_pi[i]);
kmfit1->AddTrack(1, Trk_pi[k]);
kmfit1->AddTrack(2, Trk_mu[j]);
kmfit1->AddMissTrack(3, Pmiss);
if(gamma_final<gamnumber){
RecEmcShower     *gDst_rec=(*(evtRecTrkCol->begin() + Gamid[gamma_final]))->emcShower();
kmfit1->AddTrack(4, 0.0,gDst_rec);
}
else{
kmfit1->AddTrack(4,Trk_pi0[gamma_final-gamnumber]);
}
for(int ntrk=0;ntrk<Tag_trk.size();ntrk++)kmfit1->AddTrack(5+ntrk,Tag_trk[ntrk]);
if(tagmode==13){
RecEmcShower     *g_etap=(*(evtRecTrkCol->begin() + tagid[3]))->emcShower();
kmfit1->AddTrack(8, 0.0,g_etap);
}
kmfit1->AddResonance(0, mDs, 0, 1,2,3);
if(Tag_trk.size()==2)kmfit1->AddResonance(1, mDs, 5,6);
if(Tag_trk.size()==3&&tagmode!=13)kmfit1->AddResonance(1, mDs, 5,6,7);
if(Tag_trk.size()==3&&tagmode==13)kmfit1->AddResonance(1, mDs, 5,6,7,8);
if(Tag_trk.size()==4)kmfit1->AddResonance(1, mDs, 5,6,7,8);
if(Tag_trk.size()==5)kmfit1->AddResonance(1, mDs, 5,6,7,8,9);

if(Tag_trk.size()==2)kmfit1->AddResonance(2, mDst, 4,5,6);
if(Tag_trk.size()==3&&tagmode!=13)kmfit1->AddResonance(2, mDst, 4,5,6,7);
if(Tag_trk.size()==3&&tagmode==13)kmfit1->AddResonance(2, mDst, 4,5,6,7,8);
if(Tag_trk.size()==4)kmfit1->AddResonance(2, mDst, 4,5,6,7,8);
if(Tag_trk.size()==5)kmfit1->AddResonance(2, mDst, 4,5,6,7,8,9);
kmfit1->AddFourMomentum(3, ecms);
kmfit1->Fit();
double   kmfit_chi1=kmfit1->chisq();

HepLorentzVector v1_temp1=kmfit1->pfit(0);//pi
HepLorentzVector v2_temp1=kmfit1->pfit(1);//pi
HepLorentzVector v3_temp1=kmfit1->pfit(2);//mu
HepLorentzVector v4_temp1=kmfit1->pfit(3);//nu
...
kmfit2->AddTrack(0, Trk_pi[i]);
kmfit2->AddTrack(1, Trk_pi[k]);
kmfit2->AddTrack(2, Trk_mu[j]);
kmfit2->AddMissTrack(3, Pmiss);
if(gamma_final<gamnumber){
RecEmcShower     *gDst_rec=(*(evtRecTrkCol->begin() + Gamid[gamma_final]))->emcShower();
kmfit2->AddTrack(4, 0.0,gDst_rec);
}
else{
kmfit2->AddTrack(4,Trk_pi0[gamma_final-gamnumber]);
}
for(int ntrk=0;ntrk<Tag_trk.size();ntrk++)kmfit2->AddTrack(5+ntrk,Tag_trk[ntrk]);
if(tagmode==13){
RecEmcShower     *g_etap=(*(evtRecTrkCol->begin() + tagid[3]))->emcShower();
kmfit2->AddTrack(8, 0.0,g_etap);
}
kmfit2->AddResonance(0, mDs, 0, 1,2,3);
if(Tag_trk.size()==2)kmfit2->AddResonance(1, mDs, 5,6);
if(Tag_trk.size()==3&&tagmode!=13)kmfit2->AddResonance(1, mDs, 5,6,7);
if(Tag_trk.size()==3&&tagmode==13)kmfit2->AddResonance(1, mDs, 5,6,7,8);
if(Tag_trk.size()==4)kmfit2->AddResonance(1, mDs, 5,6,7,8);
if(Tag_trk.size()==5)kmfit2->AddResonance(1, mDs, 5,6,7,8,9);

kmfit2->AddResonance(2, mDst, 0,1,2,3,4);
kmfit2->AddFourMomentum(3, ecms);
kmfit2->Fit();
double   kmfit_chi2=kmfit2->chisq();


HepLorentzVector v1_temp2=kmfit2->pfit(0);//pi
HepLorentzVector v2_temp2=kmfit2->pfit(1);//pi
HepLorentzVector v3_temp2=kmfit2->pfit(2);//mu
HepLorentzVector v4_temp2=kmfit2->pfit(3);//nu


if(kmfit_chi1<kmfit_chi2){//select minimum to make sure Ds* from whether single or signal
V4_1ft=v1_temp1;//pi
V4_2ft=v2_temp1;//pi
V4_3ft=v3_temp1;//mu
V4_4ft=v4_temp1;//nv
...
//kinimatic fit is to optimize miss mass distribution

mm2mft=V4_4ft.e()*V4_4ft.e()-V4_4ft.rho()*V4_4ft.rho();
m12ft=(V4_1ft+V4_2ft).m();
m13ft=(V4_1ft+V4_3ft).m();
m14ft=(V4_1ft+V4_4ft).m();
m23ft=(V4_2ft+V4_3ft).m();
m24ft=(V4_2ft+V4_4ft).m();
m34ft=(V4_3ft+V4_4ft).m();
m123ft=(V4_1ft+V4_2ft+V4_3ft).m();
m124ft=(V4_1ft+V4_2ft+V4_4ft).m();
m134ft=(V4_1ft+V4_3ft+V4_4ft).m();
m234ft=(V4_2ft+V4_3ft+V4_4ft).m();

HepLorentzVector p_nu_temp=V4_4ft;
p_nu_temp.setE(V4_4ft.rho());
q2ft=(V4_3ft+p_nu_temp).m2();
...
if(recmode==1){ //Ds+ -> f_0(pi+ pi-) mu nu //select closest f_0

double mf0_ini = -1;
for(int ipip=0;ipip<pinumber;ipip++){
    if(Picharge[ipip]!=tagcharge)continue;
   // if(i==ipip)continue;
for(int k=0;k<pinumber;k++){//for pi-
    if(Picharge[k]==tagcharge)continue;
    double mf0_tmp = (Pivector[ipip]+Pivector[k]).m();
    if (abs(mf0_tmp - mf0) < abs(mf0_ini - mf0)){// misjudgment exist
        mf0_ini = mf0_tmp;
for(int j=0;j<munumber;j++){//for mu+
    if(Mucharge[j]!=tagcharge)continue;


Vint idtemp=tagid;
idtemp.push_back(Piid[ipip]);
idtemp.push_back(Piid[k]);
...
HepLorentzVector Pmiss=ECMS-Pgtag_final-Pivector[ipip]-Pivector[k]-Muvector[j]; 
...
P_nu.setPhi(Pmiss.phi());
mm2m=Pmiss.e()*Pmiss.e()-Pmiss.rho()*Pmiss.rho();
m123=(Pivector[ipip]+Pivector[k]+Muvector[j]).m();
m124=(Pivector[ipip]+Pivector[k]+P_nu).m();
m134=(Pivector[ipip]+Muvector[j]+P_nu).m();
m234=(Pivector[k]+Muvector[j]+P_nu).m();
m12=(Pivector[ipip]+Pivector[k]).m();
m13=(Pivector[ipip]+Muvector[j]).m();
m14=(Pivector[ipip]+P_nu).m();
m23=(Pivector[k]+Muvector[j]).m();
m24=(Pivector[k]+P_nu).m();
m34=(Muvector[j]+P_nu).m();
...
TotID.clear();
TotID=tagid;
TotID.push_back(Piid[ipip]);
TotID.push_back(Piid[k]);
...
//double m12_temp1 = (v1_temp1+v2_temp1).m();//pi+pi-
//double m23_temp1 = (v2_temp1+v3_temp1).m();//pi-mu+
//
//int ipi = -1;
//int imu = -1;
//if (abs(m12_temp1 - mf0) > abs(m23_temp1 - mf0)) {//misjudgment exist
//    ipi = j;
//    imu = i;
//}
//else{//misjudgment inexist
//ipi = i;
//imu = j;
//}
...
if(tagcharge==1){
    pia.push_back(Pivector[ipip]);
    pib.push_back(Pivector[k]);
}
if(tagcharge==-1){
    pib.push_back(Pivector[ipip]);
    pia.push_back(Pivector[k]);
}
//eta.push_back(Etavector[i]);
mu.push_back(Muvector[j]);
