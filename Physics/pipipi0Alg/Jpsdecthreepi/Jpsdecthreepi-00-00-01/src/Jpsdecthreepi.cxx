  ...
  if ( nt1 ) m_tuple1 = nt1;
    ...
    status = m_tuple1->addItem ("em_dedxchi",            em_dedxchi);
     ...
     status = m_tuple1->addItem ("elect_vtxfit_chi2",   elect_vtxfit_chi2);
      ...
      status = m_tuple1->addItem ("Gam_fit_mass",       gammass);
      status = m_tuple1->addItem ("Gam_fit_Px",         gampx);
      status = m_tuple1->addItem ("Gam_fit_Py",         gampy);
      status = m_tuple1->addItem ("Gam_fit_Pz",         gampz);
      status = m_tuple1->addItem ("Gam_fit_E",          game);
      status = m_tuple1->addItem ("Gam_fit_Theta",      gamtheta);
      status = m_tuple1->addItem ("Gam_fit_Phi",        gamphi);
      status = m_tuple1->addItem ("Gam_fit_P",          gammom);
 ...
 vtxfit->Swim(0);
  ...
  KalmanKinematicFit * kmfit = KalmanKinematicFit::instance();
    ...
    if(chisq < 200 && ig!=-1) { 
      ...
      RecEmcShower *gTrk = (*(evtRecTrkCol->begin()+ig))->emcShower();
      kmfit->init();
      kmfit->AddTrack(0, wpip);
      kmfit->AddTrack(1, wpim);
      kmfit->AddTrack(2, wep);
    ...
    gammass    =   kmfit->pfit(4).m();
    gampx      =   kmfit->pfit(4).px();
    gampy      =   kmfit->pfit(4).py();
    gampz      =   kmfit->pfit(4).pz();
    game       =   kmfit->pfit(4).e();
    gamtheta   =   kmfit->pfit(4).theta();
    gamphi     =   kmfit->pfit(4).phi();
    gammom     =   kmfit->pfit(4).vect().mag();
