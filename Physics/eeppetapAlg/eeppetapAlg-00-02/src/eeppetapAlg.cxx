...
#endif
#include "eeppetapAlg/util/MyIsPID.h"
#include "eeppetapAlg/util/MyUtil.h"
#include "eeppetapAlg/util/MyInitIP.h"
#include "eeppetapAlg/util/MyIsGoodtrack.h"
...
#include "eeppetapAlg/eeppetapAlg.h"
...
int Ncut[10];
////////////:///////////////////////////////////////////////////////////////
eeppetapAlg::eeppetapAlg(const std::string& name, ISvcLocator* pSvcLocator) :
...
//########################################################################
StatusCode eeppetapAlg::initialize(){
...
//************************************************************************
StatusCode eeppetapAlg::execute() {
...
//    wvelm1Trk = WTrackParameter(me, elm1Trk->getZHelixE(), elm1Trk->getZErrorE());
        ...
        HepVector wplus1(5,0);
        HepVector wminus1(5,0);
        HepVector wplus2(5,0);
        HepVector wminus2(5,0);
        HepVector wplus3(5,0);
        HepVector wminus3(5,0);

        RecMdcKalTrack* cor_pip1Trk  = pip1Trk;
        RecMdcKalTrack* cor_pip2Trk  = pip2Trk;
        RecMdcKalTrack* cor_pim1Trk  = pim1Trk;
        RecMdcKalTrack* cor_pim2Trk  = pim2Trk;
        RecMdcKalTrack* cor_elp1Trk  = elp1Trk;
        RecMdcKalTrack* cor_elm1Trk  = elm1Trk;

        eeppetapAlg::calibration(cor_pip1Trk, wplus1  , 2);
        eeppetapAlg::calibration(cor_pim1Trk, wminus1 , 2);
        eeppetapAlg::calibration(cor_pip2Trk, wplus2  , 2);
        eeppetapAlg::calibration(cor_pim2Trk, wminus2 , 2);
        eeppetapAlg::calibration(cor_elp1Trk, wplus3  , 0);
        eeppetapAlg::calibration(cor_elm1Trk, wminus3 , 0);

        wvpip1Trk = WTrackParameter(mpi, wplus1  , cor_pip1Trk->getZError());
        wvpim1Trk = WTrackParameter(mpi, wminus1 , cor_pim1Trk->getZError());
        wvpip2Trk = WTrackParameter(mpi, wplus2  , cor_pip2Trk->getZError());
        wvpim2Trk = WTrackParameter(mpi, wminus2 , cor_pim2Trk->getZError());
        wvelp1Trk = WTrackParameter(me,  wplus3  , cor_elp1Trk->getZErrorE());
        wvelm1Trk = WTrackParameter(me,  wminus3 , cor_elm1Trk->getZErrorE());
...
//        RecMdcKalTrack* cor_elm1Trk = elm1Trk;
//        eeppetapAlg::calibration(cor_pip1Trk, wplus1, 2);
//        eeppetapAlg::calibration(cor_pip2Trk, wminus1, 2);
//        eeppetapAlg::calibration(cor_pim1Trk,  wplus2, 2);
//        eeppetapAlg::calibration(cor_pim2Trk,  wminus2, 2);
//        eeppetapAlg::calibration(cor_elp1Trk,  wplus3, 0);
//        eeppetapAlg::calibration(cor_elm1Trk, wminus3, 0);
//
//        wpip1 = WTrackParameter(mpi, wplus1, cor_pip1Trk->getZError());
//        wpim1 = WTrackParameter(mpi, wminus1, cor_pip2Trk->getZError());
//        wpip2 = WTrackParameter(mpi, wplus2, cor_pim1Trk->getZError());
//        wpim2 = WTrackParameter(mpi, wminus2, cor_pim2Trk->getZError());
//        welp1 = WTrackParameter(me, wplus3, cor_elp1Trk->getZErrorE());
//        welm1 = WTrackParameter(me, wminus3, cor_elm1Trk->getZErrorE());
        ...
        if(tmp_chi2_rho_4c < 200.){
            ...
            if(oksq) {
                ...
                if (ipip_etap > -1 && ipim_etap > -1) {
                    ...
                    ////////////////////////////////truthmatch////////////////////////////////
                    SmartDataPtr<Event::EventHeader> Eventheader(eventSvc(),"/Event/EventHeader");
                    Vp4 mctrue_ppip_etap_4c , mctrue_ppim_etap_4c , mctrue_pgam_etap_4c;
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
StatusCode eeppetapAlg::finalize() {
    MsgStream log(msgSvc(), name());
...
double eeppetapAlg::Get_HelixRadius(HepVector helix){
...
HepPoint3D eeppetapAlg::Get_HelixCenter(HepVector helix){
...
HepPoint3D eeppetapAlg::intersections(HepVector helix,double r){
    double xc, yc,rc;
...
double eeppetapAlg::Get_Dee(HepVector helixe,HepVector helixp,double r){
...
void eeppetapAlg::calibration(RecMdcKalTrack *trk , HepVector &wtrk_zHel, int n )
{
    ...
    ep_calerr_d2[0] = 1.0;
    ep_calerr_d2[1] = 1.191;
    ep_calerr_d2[2] = 1.046;
    ep_calerr_d2[3] = 1.0;
    ep_calerr_d2[4] = 1.177;

    em_calerr_d2[0] = 1.0;
    em_calerr_d2[1] = 1.206;
    em_calerr_d2[2] = 1.000;
    em_calerr_d2[3] = 1.0;
    em_calerr_d2[4] = 1.153;

    mup_calerr_d2[0] = 1.0;
    mup_calerr_d2[1] = 1.277;
    mup_calerr_d2[2] = 1.103;
    mup_calerr_d2[3] = 1.0;
    mup_calerr_d2[4] = 1.129;

    mum_calerr_d2[0] = 1.0;
    mum_calerr_d2[1] = 1.269;
    mum_calerr_d2[2] = 1.057;
    mum_calerr_d2[3] = 1.0;
    mum_calerr_d2[4] = 1.138;

    pip_calerr_d2[0] = 1.0;
    pip_calerr_d2[1] = 1.241;
    pip_calerr_d2[2] = 1.109;
    pip_calerr_d2[3] = 1.0;
    pip_calerr_d2[4] = 1.111;

    pim_calerr_d2[0] = 1.0;
    pim_calerr_d2[1] = 1.215;
    pim_calerr_d2[2] = 1.104;
    pim_calerr_d2[3] = 1.0;
    pim_calerr_d2[4] = 1.009;
    ...
    kp_calerr_d2[0] = 1.0;
    kp_calerr_d2[1] = 1.087;
    kp_calerr_d2[2] = 1.178;
    kp_calerr_d2[3] = 1.0;
    kp_calerr_d2[4] = 1.178;

    km_calerr_d2[0] = 1.0;
    km_calerr_d2[1] = 1.162;
    km_calerr_d2[2] = 1.131;
    km_calerr_d2[3] = 1.0;
    km_calerr_d2[4] = 1.253;

    pp_calerr_d2[0] = 1.0;
    pp_calerr_d2[1] = 1.125;
    pp_calerr_d2[2] = 1.168;
    pp_calerr_d2[3] = 1.0;
    pp_calerr_d2[4] = 1.090;

    pm_calerr_d2[0] = 1.0;
    pm_calerr_d2[1] = 1.107;
    pm_calerr_d2[2] = 1.112;
    pm_calerr_d2[3] = 1.0;
    pm_calerr_d2[4] = 1.062;
    ...
    ep_calmean_d2[0] = 0;
    ep_calmean_d2[1] = -0.055;
    ep_calmean_d2[2] = 0.447;
    ep_calmean_d2[3] = 0;
    ep_calmean_d2[4] = 0.098;

    em_calmean_d2[0] = 0;
    em_calmean_d2[1] = 0.037;
    em_calmean_d2[2] = -0.373;
    em_calmean_d2[3] = 0;
    em_calmean_d2[4] = 0.120;

    mup_calmean_d2[0] = 0;
    mup_calmean_d2[1] = -0.045;
    mup_calmean_d2[2] = 0.499;
    mup_calmean_d2[3] = 0;
    mup_calmean_d2[4] = 0.184;

    mum_calmean_d2[0] = 0;
    mum_calmean_d2[1] = 0.031;
    mum_calmean_d2[2] = -0.464;
    mum_calmean_d2[3] = 0;
    mum_calmean_d2[4] = 0.233;

    pip_calmean_d2[0] = 0;
    pip_calmean_d2[1] = -0.019;
    pip_calmean_d2[2] = 0.302;
    pip_calmean_d2[3] = 0;
    pip_calmean_d2[4] = 0.115;

    pim_calmean_d2[0] = 0;
    pim_calmean_d2[1] = 0.003;
    pim_calmean_d2[2] = -0.250;
    pim_calmean_d2[3] = 0;
    pim_calmean_d2[4] = 0.092;

    kp_calmean_d2[0] = 0;
    kp_calmean_d2[1] = -0.007;
    kp_calmean_d2[2] = 0.362;
    kp_calmean_d2[3] = 0;
    kp_calmean_d2[4] = 0.168;

    km_calmean_d2[0] = 0;
    km_calmean_d2[1] = -0.06;
    km_calmean_d2[2] = -0.361;
    km_calmean_d2[3] = 0;
    km_calmean_d2[4] = 0.211;
    pp_calmean_d2[0] = 0;
    pp_calmean_d2[1] = 0;
    pp_calmean_d2[2] = 0;
    pp_calmean_d2[3] = 0;
    ...
    if(trk->charge()>0 && n==0)
    {
        ...
        wtrk_zHel[1] = trk->getZHelixE()[1]+ep_calmean_d2[1]*sqrt(wep_zerr[1][1])+sqrt(wep_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixE()[2]+ep_calmean_d2[2]*sqrt(wep_zerr[2][2])+sqrt(wep_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixE()[3];
        wtrk_zHel[4] = trk->getZHelixE()[4]+ep_calmean_d2[4]*sqrt(wep_zerr[4][4])+sqrt(wep_zcal[2][2])*gRandom->Gaus(0,1);
    }
    ...
    if(trk->charge()<0 && n==0)
    {
        ...
        wtrk_zHel[0] = trk->getZHelixE()[0];
        wtrk_zHel[1] = trk->getZHelixE()[1]+em_calmean_d2[1]*sqrt(wem_zerr[1][1])+sqrt(wem_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixE()[2]+em_calmean_d2[2]*sqrt(wem_zerr[2][2])+sqrt(wem_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixE()[3];
        wtrk_zHel[4] = trk->getZHelixE()[4]+em_calmean_d2[4]*sqrt(wem_zerr[4][4])+sqrt(wem_zcal[2][2])*gRandom->Gaus(0,1);
    ...
    if(trk->charge()>0 && n==1)
    {
        ...
        wtrk_zHel[1] = trk->getZHelixMu()[1]+mup_calmean_d2[1]*sqrt(wmup_zerr[1][1])+sqrt(wmup_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixMu()[2]+mup_calmean_d2[2]*sqrt(wmup_zerr[2][2])+sqrt(wmup_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixMu()[3];
        wtrk_zHel[4] = trk->getZHelixMu()[4]+mup_calmean_d2[4]*sqrt(wmup_zerr[4][4])+sqrt(wmup_zcal[2][2])*gRandom->Gaus(0,1);
    ...
    if(trk->charge()<0 && n==1)
    {
        ...
        wtrk_zHel[0] = trk->getZHelixMu()[0];
        wtrk_zHel[1] = trk->getZHelixMu()[1]+mum_calmean_d2[1]*sqrt(wmum_zerr[1][1])+sqrt(wmum_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixMu()[2]+mum_calmean_d2[2]*sqrt(wmum_zerr[2][2])+sqrt(wmum_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixMu()[3];
        wtrk_zHel[4] = trk->getZHelixMu()[4]+mum_calmean_d2[4]*sqrt(wmum_zerr[4][4])+sqrt(wmum_zcal[2][2])*gRandom->Gaus(0,1);
    }
    ...
    if(trk->charge()>0 && n==2)
    {
        ...
        wtrk_zHel[1] = trk->getZHelix()[1]+pip_calmean_d2[1]*sqrt(wpip_zerr[1][1])+sqrt(wpip_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelix()[2]+pip_calmean_d2[2]*sqrt(wpip_zerr[2][2])+sqrt(wpip_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelix()[3];
        wtrk_zHel[4] = trk->getZHelix()[4]+pip_calmean_d2[4]*sqrt(wpip_zerr[4][4])+sqrt(wpip_zcal[2][2])*gRandom->Gaus(0,1);
    }
    ...
    if(trk->charge()<0 && n==2)
    {
        ...
        wtrk_zHel[0] = trk->getZHelix()[0];
        wtrk_zHel[1] = trk->getZHelix()[1]+pim_calmean_d2[1]*sqrt(wpim_zerr[1][1])+sqrt(wpim_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelix()[2]+pim_calmean_d2[2]*sqrt(wpim_zerr[2][2])+sqrt(wpim_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelix()[3];
        wtrk_zHel[4] = trk->getZHelix()[4]+pim_calmean_d2[4]*sqrt(wpim_zerr[4][4])+sqrt(wpim_zcal[2][2])*gRandom->Gaus(0,1);
    }
    ...
    if(trk->charge()>0 && n==3)
    {
        ...
        wtrk_zHel[0] = trk->getZHelix()[0];
        wtrk_zHel[1] = trk->getZHelix()[1]+kp_calmean_d2[1]*sqrt(wkp_zerr[1][1])+sqrt(wkp_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelix()[2]+kp_calmean_d2[2]*sqrt(wkp_zerr[2][2])+sqrt(wkp_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelix()[3];
        wtrk_zHel[4] = trk->getZHelix()[4]+kp_calmean_d2[4]*sqrt(wkp_zerr[4][4])+sqrt(wkp_zcal[2][2])*gRandom->Gaus(0,1);
    ...
    if(trk->charge()<0 && n==3)
    {
        ...
        wtrk_zHel[0] = trk->getZHelix()[0];
        wtrk_zHel[1] = trk->getZHelix()[1]+km_calmean_d2[1]*sqrt(wkm_zerr[1][1])+sqrt(wkm_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelix()[2]+km_calmean_d2[2]*sqrt(wkm_zerr[2][2])+sqrt(wkm_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelix()[3];
        wtrk_zHel[4] = trk->getZHelix()[4]+km_calmean_d2[4]*sqrt(wkm_zerr[4][4])+sqrt(wkm_zcal[2][2])*gRandom->Gaus(0,1);
    ...
    if(trk->charge()>0 && n==4)
    {
        ...
        wtrk_zHel[1] = trk->getZHelixP()[1]+pp_calmean_d2[1]*sqrt(wpp_zerr[1][1])+sqrt(wpp_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixP()[2]+pp_calmean_d2[2]*sqrt(wpp_zerr[2][2])+sqrt(wpp_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixP()[3];
        wtrk_zHel[4] = trk->getZHelixP()[4]+pp_calmean_d2[4]*sqrt(wpp_zerr[4][4])+sqrt(wpp_zcal[2][2])*gRandom->Gaus(0,1);
    }
    ...
    if(trk->charge()<0 && n==4)
    {
        ...
        wtrk_zHel[0] = trk->getZHelixP()[0];
        wtrk_zHel[1] = trk->getZHelixP()[1]+pm_calmean_d2[1]*sqrt(wpm_zerr[1][1])+sqrt(wpm_zcal[0][0])*gRandom->Gaus(0,1);
        wtrk_zHel[2] = trk->getZHelixP()[2]+pm_calmean_d2[2]*sqrt(wpm_zerr[2][2])+sqrt(wpm_zcal[1][1])*gRandom->Gaus(0,1);
        wtrk_zHel[3] = trk->getZHelixP()[3];
        wtrk_zHel[4] = trk->getZHelixP()[4]+pm_calmean_d2[4]*sqrt(wpm_zerr[4][4])+sqrt(wpm_zcal[2][2])*gRandom->Gaus(0,1);
    }
}
